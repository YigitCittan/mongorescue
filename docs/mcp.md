...
- **Untrusted text stays data.** Names, database names and error messages come from users, MongoDB servers or failures, so a planted instruction could reach the model. Tool text summaries therefore carry only IDs, counts, statuses and timestamps (a database name, when needed, quoted and cut to 64 characters); everything else is only in the structured JSON result.
...
- **Accountable.** Every tool call, resource read and prompt request is recorded in the audit log with the key, tool, arguments (secrets redacted), result and duration, and each key is rate limited.
...
- **Audit log.** Every tool call, resource read (`resources/read`) and prompt request (`prompts/get`) is stored with time, API key (ID and name), transport (`http`, or `stdio` for the bridge), tool, arguments (values of secret-looking keys and credentials in strings are masked), result (`ok`, `error`, `denied`, `rate_limited`), the error shown to the assistant, the duration and a `count`. The newest 10,000 entries are kept; repeated `denied` calls of one key and tool, and repeated `rate_limited` calls of one key, are merged into one entry per 10 seconds (`count` says how many calls it stands for), so refused calls cannot flush the log. REST requests made with API keys are logged there too (transport `rest`). See the last 200 under **Settings → Security → Recent API/MCP activity**, or call `GET /api/v1/audit?limit=200` with an admin key ([api.md](api.md#audit-log)).
